...
-- PK를 가리킬 수 있는 값. 참조 값
-- 예> 회원이 구매한 상품의 정보 - 회원(PK: 회원 아이디) - 주문( PK : 주문 번호, FK: 회원의 아이디, 상품의 번호) - 상품 (PK: 상품의 번호) 
...
-- 평균 월급 보다 적게 받는 사원들의 사원번호와 월급을 조회한다.
-- 모르는것 평균 월급 
SELECT AVG(SALARY)
  FROM EMPLOYEES
;
SELECT EMPLOYEE_ID
 ...
 WHERE SALARY < (SELECT AVG(SALARY)
                   FROM EMPLOYEES)
;                   
-- 수행하는 직무의 이름이 'Finance Manger'인 사원의 사원 번호와 직무 아이디를 조회한다.
SELECT JOB_ID
  FROM JOBS
...
-- Seattle 에서 근무중인 사원의 이름, 성 , 부서 번호를 조회한다. 
--1. 모르는것 SEATTLE 의 지역번호  ==> 1700
SELECT LOCATION_ID
  FROM LOCATIONS
 WHERE CITY = 'Seattle'
;
--2. 모르는것 SEATTLE 에 있는 부서들의 번호가 무엇인지 모른다.==> 지역번호가 1700 인 부서의 번호가 무엇인지 모른다.
...
--SEATTLE 에 있는 부서들의 번호가 무엇인지 안다면, 사원의 이름과 성, 부서번호를 조회할수있다.
SELECT FIRST_NAME
     , LAST_NAME
     , DEPARTMENT_ID
  FROM EMPLOYEES
...
SELECT SYSDATE (현재 시간을 가리키는 Oracle 전용 예약어)
  FROM DUAL
;
-- 오늘 기준으로 하루 전 의 날짜와 시간을 조회 (ORACLE 전용)
SELECT SYSDATE -1
  FROM DUAL
;
...
SELECT SYSDATE -12 /24
  FROM DUAL
;
--현재시간을 기준으로 15분 전의 날짜와 시간을 조회.
SELECT SYSDATE - 15 / 24 /60
  FROM DUAL
;
--현재시간을 기준으로 50초 이후의 날짜와 시간을 조회
...
SELECT TO_DATE('2026-02-13','YYYY-MM-DD')+3/24
  FROM DUAL
;
-- '2026-02-12' 기준으로 1일 후의 날짜만 조회한다.
SELECT TO_CHAR(TO_DATE('2026-02-12','YYYY-MM-DD')+1, 'DD')
  FROM DUAL
;
...
SELECT EMPLOYEE_ID
 ...
 WHERE JOB_ID  = 'AD_VP'
;
-- 15. 월봉이 7000 이상인 사원의 모든 정보를 조회한다.
SELECT EMPLOYEE_ID
	 , FIRST_NAME
	 , LAST_NAME
	 , EMAIL
...
SELECT COUNTRY_NAME
  FROM COUNTRIES
 WHERE COUNTRY_ID IN (SELECT LOCATION_ID
                        FROM DEPARTMENTS
                       WHERE DEPARTMENT_ID NOT IN (SELECT DISTINCT DEPARTMENT_ID
                                                     FROM EMPLOYEES
                                                    WHERE DEPARTMENT_ID IS NOT NULL))
...
-- 64. 대륙 별 사원의 수를 조회한다.
-- 65. 월급이 2500, 3500, 7000 이 아니며 직업이 SA_REP 이나 ST_CLERK 인 사원들의 월급과 직무 아이디를 조회한다.
-- 월급이 2500 , 3500 , 7000 이 아닌 사원들의 사원 번호와 월급을 조회한다.
SELECT EMPLOYEE_ID
     , SALARY 
  FROM EMPLOYEES
...
--직무 아이디가 SA_REP 이거나 ST_CLERK 인 사원들의 사원번호와 직무 아이디를 조회한다.
SELECT EMPLOYEE_ID
     , JOB_ID 
  FROM EMPLOYEES
...
SELECT EMPLOYEE_ID
 ...
 ORDER BY JOB_ID ASC
     , SALARY DESC
;
-- 88. 직무아이디 평균연봉을 평균연봉순으로 오름차순 정렬하여 조회한다.
SELECT JOB_ID
...
SELECT TO_CHAR(TO_DATE('20230222', 'YYYY-MM-DD')+10)
  FROM DUAL
;
...
--국가 명이 'a'로 끝나지 않는 국가의 모든 정보를 조회한다 '%a'
SELECT COUNTRY_ID
     , COUNTRY_NAME
     , REGION_ID
  FROM COUNTRIES
